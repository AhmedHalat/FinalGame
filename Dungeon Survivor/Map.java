...
public class Map{
    ...
    //Parameters: file, tiles
    //Guilds the map from the file
    //Constructor
            ...
            while(scanner.hasNextLine()) {
                ...
                if(!line.startsWith("//")){
                    ...
                    if(splitString.length >= 4){
                        ...
                        if(mappedTile.y < minY) minY = mappedTile.y;
                        if(mappedTile.x > maxX)  maxX = mappedTile.x;
                        if(mappedTile.x > maxY)  maxY = mappedTile.y;

                        if(numLayers <= mappedTile.layer)  numLayers = mappedTile.layer + 1;
    ...
    //Parameters: layer of the tile, tilexy
    ...
    //Parameters: rect, layer of tile, zooms
    //checks the collision of the tile
    ...
    //Parameters: renderer - object that render, GameObjects, zooms
    //renders the map then renders all gameobjects above it
    //returns Void
    public void render(RenderHandler renderer, GameObject[] objects, int xZoom, int yZoom){
        ...
        for(int layer = 0; layer < numLayers; layer++) 
        {
            ...
            for(int i = 0; i < objects.length; i++){
                if( i == 4) objects[i].render(renderer, xZoom, yZoom);
                else if(objects[i].getLayer() == layer)
